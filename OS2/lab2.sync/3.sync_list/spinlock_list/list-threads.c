...
void* shuffler(void* arg) {
	...
	while (1) {
		int cnt = 0;
		Node* n1 = l->first;
		pthread_spin_lock(&n1->sync);
		Node* n2 = n1->next;
		pthread_spin_lock(&n2->sync);
		while (n2->next != NULL) {
			Node* n3 = n2->next;
			pthread_spin_lock(&n3->sync);
			if (rand_r(&seed) % 100 < 5) {
				shuffle(n1);
				cnt++;
				pthread_spin_unlock(&n1->sync);
				n1 = n3;
			}
			else {
				pthread_spin_unlock(&n1->sync);
